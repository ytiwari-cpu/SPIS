...
export class ProgrammeService extends BaseService {
  ...
  async list() {
    ...
    // Any user with broad programme admin access sees all programmes.
    // This includes SuperAdmin, Admin (ADMIN.PROGRAMMES.VIEW) and
    // ProgrammeManager / custom roles (PROGRAMME.PROGRAMMES.VIEW).
    const canViewAll =
      this.hasPermission('ADMIN.PROGRAMMES.VIEW') ||
      this.hasPermission('PROGRAMME.PROGRAMMES.VIEW')
    if (canViewAll) return allProgrammes
    // Fallback: return only programmes this user created or is linked to.
