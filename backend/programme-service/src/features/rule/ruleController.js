...
export class RuleController extends BaseController {
  ...
  async addProgrammeRule() {
    try {
      const parsed = addProgrammeRuleSchema.parse(this.context.request.body)
      const data   = await this.service.addProgrammeRule(this.context.request.params.programmeId, parsed)
      this.respondCreated({ success: true, data })
    } catch (err) {
  ...
  async createVersion() {
    try {
      const parsed = createRuleVersionSchema.parse(this.context.request.body)
      const data   = await this.service.createVersion(parsed)
      this.respondCreated({ success: true, data })
    } catch (err) {
      this.respondError({ success: false, error: err.message })
    }
