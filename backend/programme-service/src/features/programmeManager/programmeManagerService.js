...
export class ProgrammeManagerService extends BaseService {
  constructor(context) {
    super(context)
    this.programmeManagerRepository = new ProgrammeManagerRepository(context)
  }
  ...
  async listByProgramme(programmeId) {
    return await this.programmeManagerRepository.findByProgramme(programmeId)
  }
  ...
  async addManager(programmeId, targetUserId) {
    ...
    await this.programmeManagerRepository.addLink({
      programme_id: programmeId, user_id: targetUserId, added_by: callerId,
    })
  ...
  async removeManager(programmeId, targetUserId) {
    const callerId = this.getUserId()
    if (!this.hasPermission('PROGRAMME.MANAGERS.MANAGE') && !this.hasPermission('ADMIN.PROGRAMMES.EDIT')) {
      const creator = await this.programmeManagerRepository.findProgrammeCreator(programmeId)
      if (creator !== callerId) {
        return { forbidden: true }
      }
    }
    await this.programmeManagerRepository.removeLink(programmeId, targetUserId)
    return { forbidden: false }
  }
  ...
  async isOwnerOrManager(programmeId, userId) {
    ...
    return this.programmeManagerRepository.isManager(programmeId, userId)
  }
}
