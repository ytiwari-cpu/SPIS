...
export class ProgrammeController extends BaseController {
  ...
  async create() {
    try {
      const data = await this.service.create(this.context.request.body)
  ...
  async update() {
    try {
      const data = await this.service.update(this.context.request.params.id, this.context.request.body)
      this.respondOk({ success: true, data })
    } catch (err) {
      this.respondError({ success: false, error: err.message })
    }
