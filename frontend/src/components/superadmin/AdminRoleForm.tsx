...
export default function AdminRoleFormContent() {
  ...
  const validateViewPermissions = (): { valid: boolean; errors: string[] } => {
    ...
    // Build a Set of all available permission keys so we can check existence
    const allAvailablePerms = new Set(
      (permissionsData?.permissions || []).map(p => p.permission_key)
    )
    ...
    // Check each group: if it has non-VIEW permissions AND a VIEW permission
    // actually exists in the system, then VIEW must also be selected.
    for (const [groupKey, perms] of Object.entries(groupedPerms)) {
      const viewKey = `${groupKey}.VIEW`
      // Only enforce if a VIEW permission actually exists for this group
      if (!allAvailablePerms.has(viewKey)) continue
